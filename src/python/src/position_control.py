...
# -*- coding: utf-8 -*-
"""
RobStride MIT Mode Position Control (Simplified and Reliable Version)
...
class PositionControllerMIT:
    def __init__(self, motor_id: int, channel='can0'):
        ...
        self.kp = 100.0  # Stiffness (Nm/rad)
    ...
    def _set_mode_raw(self, mode: int):
        """
        Use raw transmit to send the mode switch command without waiting for a response (to avoid connect timeout)
        """
        ...
        time.sleep(0.1)  # Wait for motor mode switch
    ...
    def connect(self):
        ...
        # Define motors
        motors = {
            self.motor_name: Motor(id=self.motor_id, model="rs-03")  # Modify model as needed
        }
        ...
        try:
            ...
            with self.lock:
                ...
                self.degree2radian = (self.initial_position*3.14159)/180
                print("🏠 Setting initial target to 0.0 ...")
                self.target_position = self.degree2radian  # Set to 0 radians
                
                # 3. Send the first MIT frame to hold the position
    ...
    def loop(self):
        """Control loop: continuously send MIT frames to maintain position"""
        print("🔄 Control loop started (Mode 0 @ 50Hz)")
        
        while self.running and self.connected:
            try:
                with self.lock:
                    # 1. Send MIT frame (just send)
                    self.bus.write_operation_frame(
                        self.motor_name,
                        self.target_position,
                        self.kp,
                    ...
                    # 2. Read status frame (just receive)
                    # This step is crucial to clear the CAN receive buffer to prevent overflow
                    # We can ignore the returned value because we only care about the "clear" action
                    self.bus.read_operation_frame(self.motor_name)
                    
                time.sleep(0.02)  # 50Hz control frequency
                
            except Exception as e:
                # Ignore timeouts, as they are common in read_operation_frame
    ...
    def set_kp(self, kp: float):
        ...
        if 0 <= kp <= 500:
            self.kp = kp
            print(f" -> Stiffness (Kp) set to: {self.kp:.1f}")
        else:
    ...
    def set_kd(self, kd: float):
        ...
        if 0 <= kd <= 5:
            self.kd = kd
            print(f" -> Damping (Kd) set to: {self.kd:.1f}")
        else:
    ...
    def run_interactive(self):
        ...
        print("👉 'kp <value>' (e.g., kp 20) to adjust stiffness (eliminate jitter)")
        print("👉 'kd <value>' (e.g., kd 0.8) to adjust damping (eliminate jitter)")
