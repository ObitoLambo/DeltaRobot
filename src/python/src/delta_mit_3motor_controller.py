...
# =========================================================
e  = 35.0  # End-effector geometry
f  = 157.0 # Base geometry
re = 400.0
rf = 200.0
t1 = 0.0
t2 = 0.0
...
class DeltaIKMitFkController:
    def __init__(self):
        ...
        self.motors: Dict[str, Motor] = {
            f"motor_{i}": Motor(id=i, model="rs-03")
            for i in MOTOR_IDS
        }
    ...
    def print_feedback_and_fk(self):
        with self.lock:
            ...
            # Read the current positions of all motors in degrees
            theta1 = math.degrees(self.feedback_rad["motor_1"])
            theta2 = math.degrees(self.feedback_rad["motor_2"])
            theta3 = math.degrees(self.feedback_rad["motor_3"])

            print(f"θ feedback (deg): [{theta1:.2f}, {theta2:.2f}, {theta3:.2f}]")

            # Calculate Forward Kinematics (FK)
            status, x, y, z = delta_calcForward(theta1, theta2, theta3, e, f, re, rf)
